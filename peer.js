...
chainStateMonitor.isAdjustedTimeoutRunning = false;
    ...
    for(item in arrayTXHeight){
      ...
      if(arrayTXHeight[item].peerTxHeight == txHt && arrayTXHeight[item].peerTxHash == txHsh){
        ...
        chainClipper(frankieCoin.blockHeight).then(function(){
          console.log("calling brv line 212");
...
updatePeerState = function(peer,maxHeight,chainCPH,txHt,txHsh,longPeerNonce,nodeType,oxHt,oxHsh,utcTS){
  ...
  if(chainState.activeSynch.receive != undefined){
    ...
    for(item in chainState.activeSynch.receive){
      //console.log(arrayCSReceive[item].peer)
      if(chainState.activeSynch.receive[item].peer == peer || chainState.activeSynch.receive[item].peer == false){
        ...
        if(parseInt(new Date().getTime()/1000) - parseInt(chainState.activeSynch.receive[item].utcTimeStamp) > 30){
          chainState.activeSynch.receive.splice(item, 1);
...
var adjustedTimeout = function(or = false) {
  ...
  if(chainStateMonitor.isAdjustedTimeoutRunning == true && or == false){
    console.log(" adjustedTimeout already running break");
    return;
  }else{
  ...
  setTimeout(function(){adjustedTimeout(true)}, chainState.interval);
  if(slowCounter == 14){
    slowCounter = 1;
  }
}
...
var cbBlockChainValidatorStartUp = function(isValid,replyData,replyHash){
  ...
  if(isValid == true){
    ...
    ***/
    ...
    if(isNaN(replyData)){
      console.log("NaN chain walk height is "+chainState.chainWalkHeight+" replyData "+replyData+" isVAlid "+isValid)
    }
    console.log("908 NOT VALID NEED TO PING AT replyData "+replyData+" isVAlid "+isValid);
    if(parseInt(replyData) > 2){
      ...
      BlkDB.blockRangeValidate(parseInt(chainState.chainWalkHeight),parseInt(chainState.chainWalkHeight+frankieCoin.chainRiser+1),cbBlockChainValidator,chainState.chainWalkHash,frankieCoin.chainRiser,416);

      //setTimeout(adjustedTimeout,100);
      /***
  ...
  if(isValid == true){
    ...
    ***/
          ...
          if(parseInt(replyData) == parseInt(chainState.synchronized)){
            ...
            if(rnod){
              ...
              if(chainState.chainWalkHeight == parseInt(chainStateMonitor.wasChainStuck.split(":")[0]) && parseInt(chainStateMonitor.wasChainStuck.split(":")[1]) > 0){
                ...
                console.log(chalk.bgCyan.black("1507 well, we are calling bottom chainSyncPing with "+parseInt(replyData)+" and "+parseInt(chainState.synchronized)))
                PEERS.peers[id].conn2.write(JSON.stringify({"ChainSyncPing":{Height:parseInt(replyData+1),MaxHeight:parseInt(chainState.synchronized),PeerNonce:chainState.peerNonce,GlobalHash:globalGenesisHash}}));
              }else{
                console.log(chalk.bgCyan.black("1510 well, we are calling bottom chainSyncPing with "+parseInt(replyData)+" and "+parseInt(chainState.synchronized)))
                PEERS.peers[id].conn2.write(JSON.stringify({"ChainSyncPing":{Height:parseInt(replyData),MaxHeight:parseInt(chainState.synchronized),PeerNonce:chainState.peerNonce,GlobalHash:globalGenesisHash}}));
              }
              called = true;
              localTempNode.random = "no";
              ...
              if(rnod){
                if(rnod.nodeType > 1){
                  ...
                  if(chainState.synchronized < chainState.peerNonce){
                    console.log(chalk.bgCyan.black("1514 well, we are calling bottom chainSyncPing with "+parseInt(replyData+1)+" and "+parseInt(chainState.synchronized)));
                    PEERS.peers[id].conn2.write(JSON.stringify({"ChainSyncPing":{Height:parseInt(replyData+1),MaxHeight:parseInt(chainState.synchronized),PeerNonce:chainState.peerNonce,GlobalHash:globalGenesisHash}}));
                  }else{
    ...
    if(called == false){//might leave this might change to activeSync
      ...
      }else if(chainState.chainWalkHeight > 1){
        activePing(10);
      }
...
var orderValidator = async function(start,end,or = false){
  ...
  var cbCheckChainStateOX = async function(csOrderHeight){
    ...
    if(parseInt(csOrderHeight.toString().split(":")[0]) <= end){
      ...
      //console.log("CALLED OXVLDY WITH "+start+" and "+end+" is running "+chainStateMonitor.isOxValidationRunning)
      if(start <= end){
...
var cbReset = async function(full = false){
  ...
  ////////////////////////////////////////////////////////////incoming transaction
          ...
          if(JSON.parse(data)["syncTrigger"]){
            ...
            BlkDB.addChainState("cs:transactionHeight",chainState.transactionHeight+":"+'');
            chainClipper(frankieCoin.blockHeight).then(function(){
              console.log("calling brv line 2391");
              BlkDB.blockRangeValidate(parseInt(chainState.chainWalkHeight),parseInt(chainState.chainWalkHeight+frankieCoin.chainRiser),cbBlockChainValidator,chainState.chainWalkHash,frankieCoin.chainRiser,2216);
            });
            cbReset();
          ...
          if(JSON.parse(data)["uncle"]){
            ...
            chainClipper(parseInt(frankieCoin.blockHeight - 1)).then(function(){
              console.log("calling brv line 2436");
              BlkDB.blockRangeValidate(parseInt(chainState.chainWalkHeight),parseInt(chainState.chainWalkHeight+frankieCoin.chainRiser),cbBlockChainValidator,chainState.chainWalkHash,frankieCoin.chainRiser,2216);
            });
            cbReset();
          ...
          if(JSON.parse(data)["ChainSyncPing"]){
            ...
            if(
              (
                JSON.parse(data)["ChainSyncPing"]["GlobalHash"] == globalGenesisHash
                && (parseInt(JSON.parse(data)["ChainSyncPing"]["PeerNonce"]) == chainState.peerNonce)
                && chainState.isMining == false
              ) || (
                JSON.parse(data)["ChainSyncPing"]["GlobalHash"] == globalGenesisHash
                && (parseInt(JSON.parse(data)["ChainSyncPing"]["PeerNonce"]) == 0)
                && chainState.isMining == false
              )
            ){
        ...
        if(isJSON(data.toString())){
          ...
          if(JSON.parse(data)["syncTrigger"]){
            ...
            chainClipper(JSON.parse(data)["syncTrigger"]).then(function(){
              console.log("calling brv line 3176");
    ...
    if(userInput == "M"){//M is for mine and triggers the miner
      ...
      BlkDB.getBlock(clipHeight,function(returnBlock){
        ...
        chainClipperReduce(frankieCoin.blockHeight,clipHeight).then(function(){
          ...
          console.log("calling CHainGrab 3586");
          ChainGrab()
      ...
      BlkDB.addChainState("cs:transactionHeight",chainState.transactionHeight+":"+'');
      chainClipper(frankieCoin.blockHeight).then(function(){
        console.log("calling brv line 3665");
        BlkDB.blockRangeValidate(parseInt(chainState.chainWalkHeight),parseInt(chainState.chainWalkHeight+frankieCoin.chainRiser),cbBlockChainValidator,chainState.chainWalkHash,frankieCoin.chainRiser,2216);
      });
      cbReset();
      cliGetInput();
      ...
      var reindexChain = function(peers){
        ...
        for (let id in peers) {
          ...
          if(rnod){
            if(rnod.nodeType > 1){
              PEERS.peers[id].conn.write(JSON.stringify({"ChainSyncPing":{Height:frankieCoin.getLength(),MaxHeight:parseInt(chainState.synchronized),GlobalHash:globalGenesisHash}}));
            }
          }else{
            PEERS.peers[id].conn.write(JSON.stringify({"ChainSyncPing":{Height:frankieCoin.getLength(),MaxHeight:parseInt(chainState.synchronized),GlobalHash:globalGenesisHash}}));
          }
...
var chainWalker = function(syncpoint,cbBlockChainValidatorStartUp){
  ...
  setTimeout(function(){
    ...
    if(frankieCoin.blockHeight > 1){
      console.log("calling brv line 4214");
...
var cbChainGrab = async function(data) {
  ...
  for (obj in data){
    ...
    //console.log("blockdata coming inbound "+JSON.parse(data[obj])["blockHeight"]+" vs memory "+JSON.stringify(frankieCoin.getBlock(JSON.parse(data[obj])["blockHeight"])))//verbose
    //verify block does not exist in memory
    var thisGetBlock = frankieCoin.getBlock(JSON.parse(data[obj])["blockHeight"]);
    if(typeof thisGetBlock === "undefined" || thisGetBlock === null){
      //block not in memory
      //console.log("block does not exist "+data[obj]);
      console.log(chalk.green("block loaded into memory ")+chalk.yellow(JSON.parse(data[obj])["blockHeight"]));
      ...
      chainState.topBlock = parseInt(JSON.parse(tempBlock)["blockHeight"]);

    }else{
      //block existed
      console.log(chalk.magenta("block already in memory: ")+chalk.yellow(JSON.parse(data[obj])["blockHeight"]));
      ...
      chainState.topBlock = parseInt(JSON.parse(data[obj])["blockHeight"]);
    }
  }
  //log(chalk.blue("BlocHeightPtr: "+ chalk.green(blockHeightPtr)));
  ...
  console.log("------------------------------------------------------");
  console.log(chalk.green("CALLING SUBMIT BLOCK"));
  console.log("------------------------------------------------------");
  ...
  *****/
  ...
  if(parseInt(frankieCoin.blockHeight) > 1){
    ...
    var postMiner = function(){
      ...
      if(allWaiting.length > 0){
        ...
        setTimeout(function(){
          if(countPostMinerCalled < 4){//recursion number of calls in 500 ms increments (osoese is recursion woot)
            ...
            if(allWaiting.length == allWaitingLength){
              ...
              BlkDB.addChainState("cs:transactionHeight",chainState.transactionHeight+":"+'');
              chainClipper(frankieCoin.blockHeight).then(function(){
                console.log("calling brv line 4411");
                BlkDB.blockRangeValidate(parseInt(chainState.chainWalkHeight),parseInt(chainState.chainWalkHeight+frankieCoin.chainRiser),cbBlockChainValidator,chainState.chainWalkHash,frankieCoin.chainRiser,2216);
              });
              cbReset();
        ...
        console.log("this logic makes no sense to me why post rpc ?");
        console.log("chainState.chainWalkHeight "+chainState.chainWalkHeight);
        console.log("chainState.topBlock "+chainState.topBlock);
        console.log("frankieCoin.blockHeight "+frankieCoin.blockHeight);
...
async function ChainGrab(blocknum){
  ...
  var resetOrderHeight = function(val){
    console.log(chalk.bgGreen("in chain grab setting orders state based on "+val))
    if(val != 0){
  ...
  //console.log("START UP CALLING getBlockRange with parseInt(chainState.chainWalkHeight) "+parseInt(chainState.chainWalkHeight)+" frankieCoin.chainRiser "+frankieCoin.chainRiser)
  ...
  setTimeout(adjustedTimeout, 15000);
